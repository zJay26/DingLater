...
- 默认下载到 `%USERPROFILE%\Downloads\DingLater`，每次使用独立子目录保存 ZIP 与完整解压文件。更新前备份旧程序到程序目录下的 `.dinglater-backup`，保留原程序路径、快捷方式、登录启动项和本地消息设置；应用文件替换失败会尝试恢复旧程序。
- 不连接钉钉网络，没有机器人/OpenAPI、遥测或云同步；更新请求不包含消息、账号、诊断或设备标识。
